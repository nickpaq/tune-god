...
2. **Computes a white-key target pitch** — major keys resolve to **C**, minor keys resolve to **A**, so once your samples are mapped to middle C in a DAW, the white keys play diatonically in the loop's key.
